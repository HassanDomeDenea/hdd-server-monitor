...
$e = fn(?string $v): string => htmlspecialchars(
    (string) $v,
    ENT_QUOTES,
    "UTF-8",
);
...
function duration(string $start, ?string $end): string
{
    $seconds = max(
        0,
        strtotime(($end ?? gmdate("Y-m-d H:i:s")) . " UTC") -
            strtotime($start . " UTC"),
    );
    if ($seconds < 60) {
        return $seconds . "s";
...
<div class="max-w-5xl mx-auto px-4 py-8">
    <header class="mb-8 flex items-end justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">Auto-refreshes every minute &middot; times in <?= $e(
 ...
 "down"
    ...
    <section>
        <h2 class="text-lg font-semibold mb-3">Events &mdash; last <?= $eventsDays ?> day<?= $eventsDays >
 1
     ? "s"
     : "" ?></h2>
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-x-auto">
            <?php if (!$events): ?>
                <p class="p-6 text-slate-500 text-sm">No events in the last <?= $eventsDays ?> day<?= $eventsDays >
 1
     ? "s"
     : "" ?>. 🎉</p>
            <?php else: ?>
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($events as $ev):
                        $open = $ev["resolved_at"] === null; ?>
                    <tr class="<?= $open ? "bg-red-50" : "" ?>">
                        <td class="px-4 py-3 font-medium"><?= $e(
                            $nameOf($ev["endpoint"]),
                        ) ?></td>
                        <td class="px-4 py-3"><?= $e($ev["description"]) ?></td>
                        <td class="px-4 py-3 whitespace-nowrap"><?= $e(
                            local_time($ev["started_at"]),
                        ) ?></td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-slate-500"><?= $e(
                            duration($ev["started_at"], $ev["resolved_at"]),
                        ) ?></td>
                    </tr>
                    <?php
                    endforeach; ?>
